...
Schedule2022=pd.read_csv(directory1+'Schedule2022.csv')
Schedule2021=pd.read_csv(directory1+'Schedule2021.csv')
Schedule2020=pd.read_csv(directory1+'Schedule2020.csv')
Schedule2019=pd.read_csv(directory1+'Schedule2019.csv')
Schedule2018=pd.read_csv(directory1+'Schedule2018.csv')
Schedule2017=pd.read_csv(directory1+'Schedule2017.csv')
Schedule2016=pd.read_csv(directory1+'Schedule2016.csv')
Schedule2015=pd.read_csv(directory1+'Schedule2015.csv')
Schedule2014=pd.read_csv(directory1+'Schedule2014.csv')
...
schedule['Season Year']=schedule['Season Year'].astype(str)
schedule['start_time']=schedule['start_time'].astype(str)
...
schedule=schedule.append(Schedule2023).append(Schedule2022).append(Schedule2021).append(Schedule2020).append(Schedule2019).append(Schedule2018).append(Schedule2017).append(Schedule2017).append(Schedule2016).append(Schedule2015).append(Schedule2014)
schedule=schedule[['start_time', 'away_team', 'home_team', 'away_team_score','home_team_score', 'Season Year']]
schedule['start_time']=schedule['start_time'].astype(str)
schedule['start_time']=pd.to_datetime(schedule['start_time'])
schedule['start_time'] = schedule['start_time'] - timedelta(hours=12)
schedule['start_time'] = schedule['start_time'].apply(lambda x: x.strftime('%Y-%m-%d'))
...
quarterdata['Season Year']=quarterdata['Season Year'].astype(str)
schedule['Season Year']=schedule['Season Year'].astype(str)
...
for ID in IDs:
    ...
    a=qwtemp.groupby('ID').agg({'PTS':'sum','FGM':'sum', 'FGA':'sum', 'FG3M':'sum', 'FG3A':'sum', 'FTM':'sum', 'FTA':'sum', 'OREB':'sum', 'DREB':'sum', 'REB':'sum', 'AST':'sum', 'TOV':'sum', 'STL':'sum', 'BLK':'sum', 'BLKA':'sum','PF':'sum', 'PFD':'sum'})
    a['FG3_PCT']=a['FG3M']/a['FG3A']
    a['FG_PCT']=a['FGM']/a['FGA']
    a['FT_PCT']=a['FTM']/a['FTA']
    a['TSA'] = a['FGA'] + 0.44 * a['FTA']
    a['TS_PCT']=a['PTS']/(2 *a['TSA'])
    a['eFG_PCT']=(a['FGM'] + 0.5 * a['FG3M'])/a['FGA']
...
for s in list(q4data['Season Year'].unique()):
    ...
    for i in list(q4data['HOME_TEAM_ABBREVIATION'].unique()):
        ...
        for j in range(0, len(tempscoring)):
            if j>0:
                tempmeanframe = tempscoring.groupby('HOME_TEAM_ABBREVIATION').agg({'PPTS': 'mean', 'PFGM': 'mean', 'PFGA': 'mean', 'PFG3M': 'mean', 'PFG3A': 'mean',
                     'PFTM': 'mean', 'PFTA': 'mean', 'POREB': 'mean', 'PDREB': 'mean', 'PREB': 'mean',
                     'PAST': 'mean','PTOV': 'mean', 'PSTL': 'mean', 'PBLK': 'mean', 'PBLKA': 'mean', 'PPF': 'mean', 'PPFD': 'mean'})
                tempmeanframe['PFG3_PCT'] = tempmeanframe['PFG3M'] / tempmeanframe['PFG3A']
                tempmeanframe['PFG_PCT'] = tempmeanframe['PFGM'] / tempmeanframe['PFGA']
                tempmeanframe['PFT_PCT'] = tempmeanframe['PFTM'] / tempmeanframe['PFTA']
                tempmeanframe['PTSA'] = tempmeanframe['PFGA'] + 0.44 * tempmeanframe['PFTA']
                tempmeanframe['PTS_PCT'] = tempmeanframe['PPTS'] / (2 * tempmeanframe['PTSA'])
                tempmeanframe['PeFG_PCT'] = (tempmeanframe['PFGM'] + 0.5 * tempmeanframe['PFG3M']) / tempmeanframe['PFGA']
...
modelsonlyq2=[]
for cut in range(190,250):
    loaded_model=load(directory1+"model_"+str(cut)+"_q2.dat")
    modelsonlyq2.insert(len(modelsonlyq2),loaded_model)

modelsonlyq3=[]
for cut in range(190,250):
    loaded_model=load(directory1+"model_"+str(cut)+"_q3.dat")
    modelsonlyq3.insert(len(modelsonlyq3),loaded_model)
...
for gameid in livematchups:
    ...
    for n in range(0, min(numplayershome, numplayersaway)):
        ...
        hometeam = game_url.json()['scoreboard']['games'][list(livematchups).index(gameid)]['homeTeam'][
            'teamTricode']
        playerhometemp['Team'] = hometeam
        playerawaytemp['Team'] = awayteam
    ...
    playerstats['FG3_PCT']=playerstats['FG3M']/playerstats['FG3A']
    playerstats['FG_PCT']=playerstats['FGM']/playerstats['FGA']
    playerstats['FT_PCT']=playerstats['FTM']/playerstats['FTA']
    playerstats['TSA'] = playerstats['FGA'] + 0.44 * playerstats['FTA']
    playerstats['TS_PCT'] = playerstats['PTS'] / (2 * playerstats['TSA'])
    playerstats['eFG_PCT'] = (playerstats['FGM'] + 0.5 * playerstats['FG3M']) / playerstats['FGA']
...
gamestats1 = gamestats[(gamestats['Time']=='End Q1') | (gamestats['Time']=='Q1 00:00') | (gamestats['Time']=='Q2 12:00')]
gamestats2 = gamestats[(gamestats['Time']=='Half') | (gamestats['Time']=='Q2 00:00')]
gamestats3 = gamestats[(gamestats['Time']=='End Q3') | (gamestats['Time']=='Q3 00:00')| (gamestats['Time']=='Q4 12:00')]
...
if not completetesting1.empty:
    completetesting1=completetesting1[['HFTA', 'HFG3_PCT', 'HPTS', 'HSTL', 'HREB', 'HFGM',
           'HFT_PCT', 'HFGA', 'HFG3A', 'HFG3M', 'HPF', 'HFG_PCT', 'HAST', 'HOREB','HTS_PCT','HeFG_PCT',
           ...
           'AFGM', 'AFT_PCT', 'AFGA', 'AFG3A', 'AFG3M', 'APF', 'AFG_PCT','ATS_PCT','AeFG_PCT',
...
if not completetesting3.empty:
    completetesting3=completetesting3['HFTA', 'HFG3_PCT', 'HPTS', 'HFGM','HeFG_PCT','HTS_PCT',
           'HFT_PCT', 'HFGA', 'HFG3A', 'HFG3M', 'HPF', 'HFG_PCT', 'HAST','HFTM', 'HPFGA', 'HPFG_PCT','HConferenceBinary', 'HOpponentConferenceBinary',
            'AFTA', 'AFG3_PCT', 'APTS', 'AFGM', 'AFT_PCT', 'AFGA', 'AFG3A', 'AFG3M', 'APF', 'AFG_PCT','AeFG_PCT','ATS_PCT',
           'AAST', 'AFTM','HHomeDaysRest', 'AHomeDaysRest','H>40', 'H>30', 'H>20', 'H>15', 'H>10', 'H>5','A>40', 'A>30', 'A>20', 'A>15', 'A>10', 'A>5','HomeTeam']
else:
    completetesting3=completetesting3

variables=['HFTA', 'HFG3_PCT', 'HPTS', 'HFGM','HeFG_PCT','HTS_PCT',
           'HFT_PCT', 'HFGA', 'HFG3A', 'HFG3M', 'HPF', 'HFG_PCT', 'HAST','HFTM', 'HPFGA', 'HPFG_PCT','HConferenceBinary', 'HOpponentConferenceBinary',
            'AFTA', 'AFG3_PCT', 'APTS', 'AFGM', 'AFT_PCT', 'AFGA', 'AFG3A', 'AFG3M', 'APF', 'AFG_PCT','AeFG_PCT','ATS_PCT',
           'AAST', 'AFTM','HHomeDaysRest', 'AHomeDaysRest','H>40', 'H>30', 'H>20', 'H>15', 'H>10', 'H>5','A>40', 'A>30', 'A>20', 'A>15', 'A>10', 'A>5']
...
for score in range(190,250):
    for j in range(0,len(completetesting3)):
        prob=modelsonlyq3[int(math.floor(float(score)))-190].predict_proba(completetesting3[variables][j:j+1])[0][1]
        predictiongrid.insert(len(predictiongrid),[score,prob,completetesting3['HomeTeam'].loc[j]])
    data=pd.DataFrame(predictiongrid)
    data=data.rename(columns={0:'TotalScore',1:'Prob',2:'HomeTeam'})
    predictdata=data

def pullprobability(teamhome,bettotal):
    print(predictdata[(predictdata['HomeTeam']==teamhome)&(predictdata['TotalScore'].between(bettotal,bettotal+5))])

pullprobability('HOU',219)
...
qwfilterhome=qw2[['ID','PTS']]
qwfilteraway=qw2[['ID','PTS']]
opponent['Q2IDH']=opponent['home_teamID']+'2'
opponent['Q2IDA']=opponent['away_teamID']+'2'
probdata=pd.merge(qwfilterhome,opponent,left_on='ID',right_on='Q2IDH',how='left')
probdata=pd.merge(probdata,qwfilteraway,left_on='Q2IDA',right_on='ID',how='left')
probdataq2=probdata.dropna()
probdataq2=probdataq2.rename(columns={'PTS_x':'HomeQ2','PTS_y':'AwayQ2'})
probdataq2['TotalQ2']=probdataq2['HomeQ2']+probdataq2['AwayQ2']
probdataq2=probdataq2.filter(['TotalQ2','TotalScore','Season Year'])

qwfilterhome=qw3[['ID','PTS']]
qwfilteraway=qw3[['ID','PTS']]
opponent['Q3IDH']=opponent['home_teamID']+'3'
opponent['Q3IDA']=opponent['away_teamID']+'3'
probdata=pd.merge(qwfilterhome,opponent,left_on='ID',right_on='Q3IDH',how='left')
probdata=pd.merge(probdata,qwfilteraway,left_on='Q3IDA',right_on='ID',how='left')
probdataq3=probdata.dropna()
probdataq3=probdataq3.rename(columns={'PTS_x':'HomeQ3','PTS_y':'AwayQ3'})
probdataq3['TotalQ3']=probdataq3['HomeQ3']+probdataq3['AwayQ3']
probdataq3=probdataq3.filter(['TotalQ3','TotalScore','Season Year'])

uniquetotscoreq2=list((probdataq2['TotalScore'].unique()))
uniquetotscoreq3=list((probdataq3['TotalScore'].unique()))
unique2score = list((probdataq2['TotalQ2'].unique()))
unique3score = list((probdataq3['TotalQ3'].unique()))
uniquetotscoreq2=list((probdataq2['TotalScore'].unique()))
uniquetotscoreq3=list((probdataq3['TotalScore'].unique()))
uniquetotscore=set(uniquetotscoreq2+uniquetotscoreq3)
uniquescoreq=set(unique2score+unique3score)

distribution2 = []
distribution3 = []

#Calculates with 5 point range
for i in uniquetotscoreq2:
    for j in unique2score:
        try:
            distribution2.insert(unique2score.index(j),[i,j,len(probdataq2[(probdataq2['TotalQ2']<=j+5)&(probdataq2['TotalQ2']>=j-5)&(probdataq2['TotalScore']>=i)]),len(probdataq2[(probdataq2['TotalQ2']<=j+5)&(probdataq2['TotalQ2']>=j-5)]),len(probdataq2[(probdataq2['TotalQ2']<=j+5)&(probdataq2['TotalQ2']>=j-5)&(probdataq2['TotalScore']>=i)])/len(probdataq2[(probdataq2['TotalQ2']<=j+5)&(probdataq2['TotalQ2']>=j-5)])])
        except:
            pass
for i in uniquetotscoreq3:
    for j in unique3score:
        try:
            distribution3.insert(unique3score.index(j),[i,j,len(probdataq3[(probdataq3['TotalQ3']<=j+5)&(probdataq3['TotalQ3']>=j-5)&(probdataq3['TotalScore']>=i)]),len(probdataq3[(probdataq3['TotalQ3']<=j+5)&(probdataq3['TotalQ3']>=j-5)]),len(probdataq3[(probdataq3['TotalQ3']<=j+5)&(probdataq3['TotalQ3']>=j-5)&(probdataq3['TotalScore']>=i)])/len(probdataq3[(probdataq3['TotalQ3']<=j+5)&(probdataq3['TotalQ3']>=j-5)])])
        except:
            pass

distrq2=pd.DataFrame(distribution2).rename(columns={0:'TotalScore',1:'TotalQ3',2:'Freq',3:'Total',4:'Probability'})
distrq3=pd.DataFrame(distribution3).rename(columns={0:'TotalScore',1:'TotalQ3',2:'Freq',3:'Total',4:'Probability'})
...
for i in uniquetotscoreq2:
    for j in unique2score:
        try:
            distribution2exact.insert(unique2score.index(j),[i,j,len(probdataq2[(probdataq2['TotalQ2']==j)&(probdataq2['TotalScore']>=i)]),len(probdataq2[(probdataq2['TotalQ2']==j)&(probdataq2['TotalQ2']>=j)]),len(probdataq2[(probdataq2['TotalQ2']==j)&(probdataq2['TotalScore']>=i)])/len(probdataq2[(probdataq2['TotalQ2']==j)])])
        except:
            pass
for i in uniquetotscoreq3:
    for j in unique3score:
        try:
            distribution3exact.insert(unique3score.index(j),[i,j,len(probdataq3[(probdataq3['TotalQ3']==j)&(probdataq3['TotalScore']>=i)]),len(probdataq3[(probdataq3['TotalQ3']==j)&(probdataq3['TotalQ3']>=j)]),len(probdataq3[(probdataq3['TotalQ3']==j)&(probdataq3['TotalScore']>=i)])/len(probdataq3[(probdataq3['TotalQ3']==j)])])
        except:
            pass

distrq2exact=pd.DataFrame(distribution2exact).rename(columns={0:'TotalScore',1:'TotalQ3',2:'Freq',3:'Total',4:'Probability'})
distrq3exact=pd.DataFrame(distribution3exact).rename(columns={0:'TotalScore',1:'TotalQ3',2:'Freq',3:'Total',4:'Probability'})

distrq2[(distrq2['TotalScore']==208)&(distrq2['Probability']>0.6)]
distrq3[(distrq3['TotalScore']==208)&(distrq3['TotalQ3']==154)]
min(distrq3[(distrq3['TotalScore']==208)&(distrq3['Probability']>0.6)]['TotalQ3'])